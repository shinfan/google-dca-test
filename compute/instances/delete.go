...
func delete(projectID string, zone string, resourceID string) (*compute.Operation, error) {
	...
	resp, err := computeService.Instances.Delete(projectID, zone, resourceID).Context(ctx).Do()
	if err != nil {
		return nil, fmt.Errorf("computeService.Instances.Delete: %v", err)
	}
	return resp, nil
}
