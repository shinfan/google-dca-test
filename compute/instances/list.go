...
func list(projectID string, zone string) (*compute.InstanceList, error) {
	...
	resp, err := computeService.Instances.List(projectID,zone).Do()
	if err != nil {
		return nil, fmt.Errorf("computeService.Instances.List: %v", err)
	}
	return resp, nil
}
